...
class AdVantageBot:
    ...
    async def show_main_menu(self, update_or_query, user):
        ...
        # Only administrators receive this button.
        ...
        text = (
            ...
            "Advantage is a digital rewards and growth platform "
            "designed to help you discover opportunities, complete "
            "tasks, earn rewards, build your digital presence, "
            "and grow through referrals.\n\n"

            "💰 <b>Earn Rewards</b>\n"
            "📋 <b>Complete Tasks</b>\n"
            "📱 <b>Grow Your Social Presence</b>\n"
            "👥 <b>Refer & Earn</b>\n"
            "💎 <b>Premium Growth</b>\n"
            "🏆 <b>Track Your Progress</b>\n\n"

            "🌍 <b>Built for everyone. Designed to grow globally.</b>\n\n"
    ...
    async def handle_callback(
        ...
        try:
            ...
            if data == "profile":
                ...
                await self.complete_social_task(
                    query,
                    db,
                    user,
                    data.split("_")[-1]
            ...
            # ------------------------------------------------
                ...
                await self.show_pending_withdrawals(
                    query,
                    db,
                    user
                )
    ...
    async def show_ads(
        ...
        if daily_views >= Config.MAX_ADS_PER_DAY:
            ...
            text = (
                f"⚠️ Daily ad limit reached "
                f"({Config.MAX_ADS_PER_DAY}). "
                "Come back tomorrow."
            )
            ...
            text = (
                ...
                "Open the advertiser link, then press "
                "the claim button."
            )
            ...
            markup = InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(
                            "✅ Claim Reward",
                            callback_data=f"ad_watched_{ad.id}"
                        )
                    ],
                    [
                        InlineKeyboardButton(
    ...
    async def ad_watched(
        ...
        # ----------------------------------------------------
        # PREVENT RAPID DUPLICATE CLAIMS
        # ----------------------------------------------------
    ...
    async def show_social_menu(
            ...
            if not done:
                ...
                buttons.append(
                    [
                        InlineKeyboardButton(
                            (
                                f"📱 {task.platform.title()} "
                                f"• +{task.points_reward} pts"
                            ),
                            callback_data=(
                                f"social_task_{task.id}"
                            ),
                        )
                    ]
                )
        ...
        text = (
            ...
            f"👥 Tasks completed: "
            f"<b>{profile.followers_gained}</b>\n\n"

            "Complete genuine social actions "
            "and submit the task in the bot."
        )
    ...
    async def complete_social_task(
        self,
        query,
        db,
        user,
    ...
    async def show_tasks(
        ...
        text = (
            ...
            "Daily money-making tasks can be added "
            "by an administrator."
        )
    ...
    async def show_payment_menu(
        ...
        text = (
            ...
            "Deposits remain pending until "
            "an administrator verifies payment."
        )
    ...
    async def show_referral(
        ...
        text = (
            ...
            f"💰 Bonus: "
            f"<b>{money(Config.REFERRAL_BONUS)}</b>\n"
    ...
    async def show_admin_panel(
        ...
        # IMPORTANT:
        # This prevents normal users from opening
        # the admin panel even if they manually send
        # the callback data.
        ...
        try:
            ...
            await self.application.bot.send_message(
                ...
                (
                    ...
                    f"{money(w.amount)} has been approved."
                ),
            )
    ...
    async def reject_withdrawal(
        ...
        try:
            ...
            await self.application.bot.send_message(
                w.user_id,
                (
                    f"↩️ Your withdrawal of "
                    f"{money(w.amount)} was rejected "
                    "and refunded."
                ),
            )
        ...
        try:
            ...
            if command == "deposit":
                ...
                await update.message.reply_text(
                    ...
                    "An admin must verify it before "
            ...
            # =================================================
                ...
                daily_total = sum(
                    w.amount for w in daily
                )

                if (
                    daily_total + amount
                    > Config.MAX_WITHDRAWAL_DAILY
                ):

                    await update.message.reply_text(
                        f"❌ Daily withdrawal limit "
                        f"is {money(Config.MAX_WITHDRAWAL_DAILY)}."
                    )
                ...
                await update.message.reply_text(
                    ...
                    f"#{w.id if w.id else 'pending'} "
                    f"created for {money(amount)}.\n"
                    "Funds are reserved while the "
                    "admin processes it."
                )
            ...
            # =================================================
                ...
                await update.message.reply_text(
                    f"✅ Credited {money(amount)} "
                    f"to user {target.telegram_id}."
                )
    ...
    async def admin_add_ad(
        ...
        try:
            ...
            await update.message.reply_text(
                "❌ Reward must be a number and "
                "MaxViews must be a whole number."
            )
    ...
    def run(self):
        ...
        # Handle commands that are not registered
        # above through handle_message.
        self.application.add_handler(
            MessageHandler(
                filters.COMMAND,
                self.handle_message
