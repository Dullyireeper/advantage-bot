import os
from dotenv import load_dotenv

load_dotenv()


class Config:
    BOT_TOKEN = os.getenv("BOT_TOKEN", "").strip()

    ADMIN_IDS = [
        int(x.strip())
        for x in os.getenv("ADMIN_IDS", "").split(",")
        if x.strip().isdigit()
    ]

    BOT_USERNAME = os.getenv("BOT_USERNAME", "AdVantageBot").strip().lstrip("@")

    DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///bot.db").strip()

    EXCHANGE_RATE = float(os.getenv("EXCHANGE_RATE", "125"))

    # These are YOUR internal accounting values for direct-sold ads.
    # They do not mean an external ad network will pay you automatically.
    AD_REVENUE_PER_VIEW = float(os.getenv("AD_REVENUE_PER_VIEW", "0.008"))
    USER_REWARD_PER_VIEW = float(os.getenv("USER_REWARD_PER_VIEW", "0.002"))

    REFERRAL_BONUS = float(os.getenv("REFERRAL_BONUS", "0.25"))

    MIN_WITHDRAWAL = float(os.getenv("MIN_WITHDRAWAL", "5"))
    MAX_WITHDRAWAL_DAILY = float(os.getenv("MAX_WITHDRAWAL_DAILY", "20"))

    MAX_ADS_PER_DAY = int(os.getenv("MAX_ADS_PER_DAY", "5"))
    MAX_TASKS_PER_DAY = int(os.getenv("MAX_TASKS_PER_DAY", "10"))
    MAX_GAMES_PER_DAY = int(os.getenv("MAX_GAMES_PER_DAY", "5"))

    MPESA_PAYBILL = os.getenv("MPESA_PAYBILL", "")
    MPESA_ACCOUNT = os.getenv("MPESA_ACCOUNT", "")

    # Optional SMTP settings for email notifications/verification later.
    SMTP_HOST = os.getenv("SMTP_HOST", "")
    SMTP_PORT = int(os.getenv("SMTP_PORT", "587"))
    SMTP_USER = os.getenv("SMTP_USER", "")
    SMTP_PASSWORD = os.getenv("SMTP_PASSWORD", "")
    SMTP_FROM = os.getenv("SMTP_FROM", "")

    VERSION = "5.0.0"

    @classmethod
    def validate(cls):
        if not cls.BOT_TOKEN:
            raise RuntimeError(
                "BOT_TOKEN is missing. Add it to Railway Variables."
            )

        if not cls.ADMIN_IDS:
            raise RuntimeError(
                "ADMIN_IDS is missing. Example: ADMIN_IDS=123456789"
            )

        if cls.USER_REWARD_PER_VIEW < 0:
            raise RuntimeError("USER_REWARD_PER_VIEW cannot be negative.")

        if cls.AD_REVENUE_PER_VIEW < cls.USER_REWARD_PER_VIEW:
            raise RuntimeError(
                "AD_REVENUE_PER_VIEW must be >= USER_REWARD_PER_VIEW."
            )

        if cls.MIN_WITHDRAWAL <= 0:
            raise RuntimeError("MIN_WITHDRAWAL must be greater than 0.")

        if cls.MAX_WITHDRAWAL_DAILY < cls.MIN_WITHDRAWAL:
            raise RuntimeError(
                "MAX_WITHDRAWAL_DAILY must be >= MIN_WITHDRAWAL."
            )
